...
Query get_nodal_catalog with mode "nodal" for each needed capability before constructing the graph; browse its paginated compact index only when discovery is needed. Each node produces cumulative state: object results are merged into `$run`, while scalar and array results are available as `$run.$result`. Use `$nodes.last` for the latest state and `$('Node label')` for a specific node's state. Call list_flow_resources when the snippet needs an accessible workspace resource or another published snippet. Provide args as a comma-separated list of unique JavaScript identifiers available through `$run.$input`. The graph must contain the canonical FUNCTION entry node with id "__system_function", name "FUNCTION", and system "function". Build the executable sequence from that node. Puppetflow validates the function graph and compiles its JavaScript body server-side with the declared arguments. Do not provide generated code or invent resource IDs. Library snippets are locked and must be duplicated outside MCP before editing.
TEXT,
            'inputSchema' => [
                'type' => 'object',
                'additionalProperties' => false,
